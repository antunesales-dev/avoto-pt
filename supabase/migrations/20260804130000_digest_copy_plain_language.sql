-- Textos do resumo do dia em linguagem clara (sem jargão "digest" / nomes de jobs)
-- Lógica idêntica a 20260803230000_digest_multisection; só copy de title/summary.
...
begin
  ...
  if v_n_inv > 0 then
    v_parts := array_append(
      v_parts,
      v_n_inv::text || case when v_n_inv = 1 then ' investimento público' else ' investimentos públicos' end
    );
  end if;
  ...
  if cardinality(v_parts) = 0 then
    ...
    v_summary :=
      'Neste dia ainda não há leis/votações, despesas ou investimentos com essa data nos dados '
      || 'oficiais importados. Quando houver actividade sincronizada, o resumo aparece aqui.';
  else
...
comment on function public.generate_daily_digest(date) is
  'Resumo do dia multi-secção (iniciativas + despesas + investimentos). Factos na BD; sem AI.';

-- Prefixar títulos já gravados (sem reprocessar o JSON dos items)
update public.daily_digests
set title = 'Resumo do dia · ' || title
where title not ilike 'resumo do dia%';
